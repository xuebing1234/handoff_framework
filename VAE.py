...
##single task, multi task, and semi-supervised
def weights_init(layer):
    if type(layer) == nn.Linear:
        torch.nn.init.orthogonal_(layer.weight)
class DNN(nn.Module):
    def __init__(
        self, input_dimension=64, hidden_layer_width=1000, n_epochs=50, 
        number_of_labels=1, number_of_classes=[4,2,2], weight=[3,3,3], device=None
    ): 
        super().__init__()
        #multi-task 
        ...
        self.net = nn.Sequential(
            torch.nn.Linear(input_dimension, self.hidden_layer_width),
            torch.nn.ReLU(),
            torch.nn.Linear(self.hidden_layer_width, self.hidden_layer_width),
            torch.nn.ReLU(),
            torch.nn.Linear(self.hidden_layer_width, self.latent_dimension),
        ).to(device)
        
        self.out = nn.ModuleList([nn.Linear(self.latent_dimension, 
                                            number_of_classes[i]) for i in range(number_of_labels)])
        self.number_of_classes = number_of_classes
        self.optimizer = optim.Adam(
           list(self.net.parameters()) +list(self.out.parameters()) , lr=1e-4
        )
    ...
    def pred_loss(self,targets,out_encoder):
        ...
        for label in range(self.number_of_labels): 
            ...
            if torch.isnan(target).all():
                ...
                preds =  self.out[label](out_encoder)[~nans]
    ...
    def forward(self, train_data):
        return self.net(train_data)
    def compute_loss(self, data, targets):
        out_encoder = self.forward(data)
        return self.pred_loss(targets,out_encoder)
    def predict_proba(self,test_data):
        preds = []
        with torch.no_grad():
            for label in range(self.number_of_labels): 
                curr_pred = self.out[label](self.net(
                            torch.Tensor(test_data).to(self.device)
                ))
                preds.append(self.softmax(curr_pred).detach().cpu().numpy())
        return preds
    def fit(self,data,train_label,train_mask
                ):
        ...
        train_mask = torch.Tensor(train_mask[indices_train])
        ...
        for epoch in trange(self.n_epochs, desc='epochs',leave=False):
            epoch_train_loss=[]
            train_set = torch.utils.data.TensorDataset(train_data, train_mask, train_label)
        ...
        if self.plot:
            ...
            for i, loss in enumerate(['combined_loss']):
...
class DisentangledVAE(nn.Module):
    ...
    def compute_loss(self, data,targets,mask):
        ...
        original_KL = self._kl_normal_loss(
            out_encoder[..., self.starting_index: self.latent_dimension],
            out_encoder[..., self.latent_dimension +self.starting_index :],
        )
        ...
        neg_bound =  self.pred_weight*prediction_losses+ recon_loss*self.recon_weight + original_KL*self.KL_weight \
        + tc_loss * self.beta 
        # + disentangled label + mmd
    ...
    def fit(self,data,train_label,train_mask
        ...
        for epoch in trange(self.n_epochs, desc='epochs',leave=False):
            ...
            for i, batch_data in enumerate(train_loader,0):
                data,mask,label = batch_data
                self.optimizer.zero_grad()
                loss = self.compute_loss(data=data.to(self.device),targets = label.to(self.device),mask = mask.to(self.device))
                loss.backward()
